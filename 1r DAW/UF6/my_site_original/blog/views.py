...
def starting_page(request):
    sorted_posts = sorted(all_posts["posts"], key=lambda x: x['date'], reverse=True)
    return render(request, template_name='my_site/index.html',
                  context={'posts': sorted_posts})


def posts(request):
    return render(request, template_name='my_site/all_posts.html', context={'posts': all_posts["posts"]})
...
def author_detail(request, author_id):
    ...
    author_slug = slugify(f"{author.first_name}-{author.last_name}")
    return render(request, 'my_site/author_detail.html', {'author': author, 'slug': author_slug})
