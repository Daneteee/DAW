...
class Input_:
    ...
    def input_int(message, min_=0, max_=0, not_limit=True, valid_return=False):
        """Mètode per introduir un enter.

        Args:
            message (str): Missatge a mostrar.
            min_ (int, optional): Valor mínim. Defaults to 0.
            max_ (int, optional): Valor màxim. Defaults to 0.
            not_limit (bool, optional): Sense valors límit. Defaults to True.
            valid_return (bool, optional): Acceptar l'entrada d'"enters" si activo. Defaults to False.


        Returns:
            int: El nombre o 0 si es pressiona enter.
        """
        v = 0
        ok = False
        while not ok:
    ...
    def input_password(message):
        """Introduim una contrasenya i comprovem que acompleixi tots els requisits.

        Args:
            message (str): Missatge a mostrar.

        Returns:
            str: Retornem la contrasenya.
        """
        is_valid = False
        passwd = ""
        while not is_valid:
            # passwd = getpass(message + ": ")
            passwd = input(message + ": ")
    ...
    def input_date(message, mmyy=False):
        """Comprovem que s'introdueixi una data vàlida.

        Args:
            message (str): Missatge a mostrar.
            mmyy (bool, optional): Indica si s'ha d'utilitzar el format mm-yy. Per defecte és False.
    ...
    def input_ok(message):
        """Validem que s'introdueixi un valor booleà.

        Args:
            message (str): Missatge a mostrar.
        Returns:
            bool: True o False depenent de l'elecció de l'usuari.
        """
        entry = input(message)
        if entry.lower() in ["s", "si", "y", "yes", "ok"]:
            return True
        else:
            return False
    ...
    def input_digits(num_digits=10):
        """Validem que s'introdueixin un cert nombre de dígits. 

        Args:
            num_digits (int, optional): Longitud que hauràn de tenir els digits totals. Defaults to 10.

        Returns:
            str: Els digits passats a string.
        """
        is_valid = False
        digits = ""
        while not is_valid:
